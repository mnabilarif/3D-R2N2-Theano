...
class Net(object):
    ...
    def __init__(self, random_seed=dt.datetime.now().microsecond, compute_grad=True):
        ...
        #a property to store net's latest hidden state
        self.s_last_shared = None 
    ...
    def load(self, filename, ignore_param=True):
        ...
        for param_idx, param in enumerate(self.params):
            try:
                ...
                #the last one is the latest hidden state(index start from 0)
                if succ_ind == len(self.params):
                    self.s_last_shared.set_value(params_cpu[succ_ind])
                succ_ind += 1
            except IndexError:
                if ignore_param:
